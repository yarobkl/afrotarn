...
function ClickCollectContent({ products, list, add, remove, clear }: { products: Product[]; list: ListState; add: (product: Product) => void; remove: (product: Product) => void; clear: () => void }) {
  ...
  const requestSubject = encodeURIComponent('Demande de retrait AfroTarn')
  const requestBody = encodeURIComponent(`Bonjour AfroTarn,\n\nJe souhaite préparer un retrait avec les articles suivants :\n${selected.map(product => `- ${product.name} x${list[product.id]}${product.priceCents !== null ? ` (${money(product.priceCents)} / unité)` : ''}`).join('\n')}\n\nPréférence de paiement : ${paymentLabel}.\n\nMerci de me confirmer la disponibilité et le retrait en boutique.\n`)
  const requestMailto = `mailto:${shop.email}?subject=${requestSubject}&body=${requestBody}`

  function addFromList(product: Product) {
    const quantity = list[product.id] || 0
    const max = product.stockMode === 'tracked' ? Math.max(0, product.stockQuantity ?? 0) : 99
    if (quantity >= max) {
      setNotice(`Stock maximum atteint pour ${product.name} (${max}).`)
      return
    }
    add(product)
    setNotice('')
  ...
  function promptPayment() {
    setNotice('Choisissez Apple Pay, Google Pay ou carte bancaire avant de finaliser la demande.')
  }
  ...
  return <>
    ...
    <section className="section list-layout commerce-v2-list-layout"><div className="list-panel"><div className="list-panel-head"><div><span className="kicker">1 · MA SÉLECTION</span><h2>{count ? `${count} article${count > 1 ? 's' : ''}` : 'Votre liste est vide'}</h2></div>{count > 0 && <button className="clear-button" onClick={() => { clear(); setPayment(null); setNotice('Liste vidée.') }}>Vider</button>}</div>{selected.length ? <><div className="list-items">{selected.map(product => <div className="list-item" key={product.dbId}><img src={product.image} alt="" onError={handleImageError} /><div className="list-copy"><strong>{product.name}</strong><span>{product.status}{product.priceCents !== null ? ` · ${money(product.priceCents)}` : ''}</span></div><div className="qty-control"><button onClick={() => { remove(product); setNotice('') }} aria-label={`Retirer un ${product.name}`}><Minus size={16} /></button><b>{list[product.id]}</b><button onClick={() => addFromList(product)} aria-label={`Ajouter un ${product.name}`}><Plus size={16} /></button></div></div>)}</div><div className="checkout-payment-step"><div className="checkout-payment-head"><div><span className="kicker">2 · PAIEMENT</span><strong>Comment souhaitez-vous payer ?</strong><p>Votre choix est enregistré dans la demande. Le débit Stripe sera activé séparément.</p></div></div><div className="checkout-payment-methods" role="radiogroup" aria-label="Choisir un paiement"><button role="radio" aria-checked={payment === 'apple'} className={`checkout-payment-method apple ${payment === 'apple' ? 'is-selected' : ''}`} onClick={() => choosePayment('apple')}>Apple Pay</button><button role="radio" aria-checked={payment === 'google'} className={`checkout-payment-method google ${payment === 'google' ? 'is-selected' : ''}`} onClick={() => choosePayment('google')}>Google Pay</button><button role="radio" aria-checked={payment === 'card'} className={`checkout-payment-method card ${payment === 'card' ? 'is-selected' : ''}`} onClick={() => choosePayment('card')}><CreditCard size={18} /> Carte bancaire</button></div><div className="checkout-payment-note"><BadgeCheck size={15} /><span>Aucun débit n’est effectué tant que Stripe n’est pas activé.</span></div>{notice && <div className="checkout-payment-note" role="status"><BadgeCheck size={15} /><span>{notice}</span></div>}</div></> : <div className="empty-list"><ShoppingBag size={32} /><p>Ajoutez d’abord vos produits.</p><Link className="button button-dark" to="/produits">Explorer les produits</Link></div>}</div><aside className="confirmation-card"><span className="kicker light">3 · CONFIRMATION</span><h2>Finaliser</h2><p>En attendant l’encaissement Stripe, la finalisation envoie une demande de retrait complète à AfroTarn.</p>{count > 0 && <div className="selected-payment-summary"><span>Paiement choisi</span><strong>{paymentLabel}</strong></div>}{!count ? <Link className="button button-light full" to="/produits"><ShoppingBag size={18} /> Choisir mes produits</Link> : payment ? <a className="button button-light full" href={requestMailto}><Mail size={18} /> Envoyer ma demande</a> : <button className="button button-light full" onClick={promptPayment}><CreditCard size={18} /> Choisir le paiement</button>}<a className="button button-outline-light full" href={`tel:${shop.phoneHref}`}><Phone size={18} /> Appeler la boutique</a></aside></section>
  </>
}
